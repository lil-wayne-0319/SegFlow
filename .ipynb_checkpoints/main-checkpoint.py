...
def parsing_args(c):
    ...
    parser.add_argument('--wtconv_enable', action='store_true', default=False,
                        help='training condition with segment mask.')
    parser.add_argument('--save_enable', action='store_true', default=False,
                        help='save images when testing.')
    parser.add_argument('--seg_enable', action='store_true', default=False,
                        help='training condition with segment mask.')
    ...
    if c.dataset == 'mvtec':
        ...
        setattr(c, 'data_path', 'F:/project_AD/MVTec')
        ...
        setattr(c, 'data_path', '/home/deeplearn/workspace/datasets/MIAD')
        if c.class_names == ['all']:
            setattr(c, 'class_names', MIAD_CLASS_NAMES)
        
    c.input_size = (256, 256) if c.class_name == 'transistor' else (512, 512)
    # c.input_size = (128, 128) if c.class_name == 'witness_mark' else (512, 512)
