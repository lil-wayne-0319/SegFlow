...
class subnet_conv_ln_wtconv(nn.Module):

    def __init__(self, dim_in, dim_out):
        super().__init__()
        dim_mid = dim_in
        # self.conv1 = nn.Conv2d(dim_in, dim_mid, 3, 1, 1)
        self.conv_dw_1 = WTConv2d(dim_in, dim_mid, kernel_size=5, wt_levels=3)
        self.ln = nn.LayerNorm(dim_mid)
        self.relu = nn.ReLU(True)
        self.conv2 = nn.Conv2d(dim_mid, dim_out, 3, 1, 1)
        # self.conv_dw_2 = WTConv2d(dim_mid, dim_out, kernel_size=5, wt_levels=3)

    def forward(self, x):
        # out = self.conv1(x)
        out = self.conv_dw_1(x)
        out = self.ln(out.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        out = self.relu(out)
        out = self.conv2(out)
...
def single_parallel_flows_wtconv(c_feat, c_cond, n_block, clamp_alpha, subnet=subnet_conv_ln_wtconv):
    flows = Ff.SequenceINN(c_feat, 1, 1)
    print('Build parallel flows: channels:{}, block:{}, cond:{}'.format(c_feat, n_block, c_cond))
    for k in range(n_block):
        flows.append(Fm.AllInOneBlock, cond=0, cond_shape=(c_cond, 1, 1), subnet_constructor=subnet, affine_clamping=clamp_alpha,
...
def build_flow_model_wtconv(c, c_feats):
    c_conds = c.c_conds
    n_blocks = c.parallel_blocks
    clamp_alpha = c.clamp_alpha
    parallel_flows = []
    for c_feat, c_cond, n_block in zip(c_feats, c_conds, n_blocks):
        parallel_flows.append(
            single_parallel_flows_wtconv(c_feat, c_cond, n_block, clamp_alpha, subnet=subnet_conv_ln_wtconv))
